...
def get_threads(num):
    url = 'http://social.technet.microsoft.com/Forums/en-US/sqlreportingservices/threads?outputAs=xml&page=%d' % num
...
def retrieve_all_threads():
    ...
    with con:
        ...
        try:
            ...
            cur.execute(
                "CREATE TABLE Comments(ROWID INTEGER PRIMARY KEY, tid INT, user TEXT, comment TEXT, commentDate TEXT, commentScore INT);")
        ...
        while True:
            threads, threads_creator, threads_replies, threads_views, threads_url, threads_url_incomplete = get_threads(
                forum_page)
            ...
            for i in xrange(0, len(threads)):
                ...
                thread_comments, thread_users, thread_comments_date = retrieve_all_comments_and_users_in_thread(
                    threads_url[i])
                user_comment_score = get_score(threads_url[i])

                cur.execute("INSERT INTO Threads(id, title, views, creator, replies, url) VALUES(?, ?, ?, ?, ?, ?);", (
                    tid, threads[i].text, threads_views[i], threads_creator[i][0].text, threads_replies[i],
                    threads_url_incomplete[i].text))
                ...
                for i in xrange(0, len(thread_users)):
                    ...
                    cur.execute(
                        "INSERT INTO Comments(tid, user, comment, commentDate, commentScore) VALUES(?, ?, ?, ?, ?);", (
                            tid, thread_users[i][0].text, thread_comments[i].text_content(), thread_comments_date[i].text,
                            user_comment_score[i]))
